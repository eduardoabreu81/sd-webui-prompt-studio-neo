...
- **Gelbooru tag list** — 235k tags with aliases, the spelling Anima's model card asks for; see [Tag Lists](#-tag-lists)
...
- Added the **Gelbooru tag list** (`gelbooru.csv`, 235.255 tags, 5,1 MB) with aliases and a matching colormap entry. Anima's model card recommends the Gelbooru spelling when a tag differs between the two sites: Gelbooru keeps `eyes_closed`, `sole_female`, `bellybutton` and `hair_past_waist` as distinct tags where Danbooru aliases them away, so they were previously unreachable from autocomplete. Generated by [`tools/build_gelbooru_tags.py`](tools/build_gelbooru_tags.py), which also keeps Gelbooru's `ambiguous` tags — the flag is set on `long_hair`, `black_hair`, `gloves` and much of the core vocabulary, so excluding them produces an unusable list
- Fixed a Gelbooru alias offering a different tag than the one typed: the upstream alias table lists pairs the site does not enforce, where both sides exist as separate tags. Searching `eyes_closed` returned `closed_eyes`, and `sole_female` — the 9th most used tag on Gelbooru — was answered with `1girl`. An alias that is itself a tag in the list is now dropped
...
- **Instant suggestions** as you type, sourced from Danbooru, Gelbooru, e621, or merged lists
...
| `gelbooru.csv` | Gelbooru tags + aliases | Anima (its model card recommends Gelbooru tag spellings) |
...
`gelbooru.csv` is generated by [`tools/build_gelbooru_tags.py`](tools/build_gelbooru_tags.py) — run it to rebuild from the current upstream snapshots. See [Tag data sources](#tag-data-sources) for provenance.
...
- **[dbr-e621-lists-archive](https://github.com/DraconicDragon/dbr-e621-lists-archive)** and **[Gelbooru-Tags-Full](https://huggingface.co/datasets/DraconicDragon/Gelbooru-Tags-Full_2026-06-11)** by **[DraconicDragon](https://github.com/DraconicDragon)** — Danbooru, e621 and Gelbooru tag snapshots (Unlicense / Apache-2.0)
- **[deepghs/site_tags](https://huggingface.co/datasets/deepghs/site_tags)** — Gelbooru tag aliases, licensed **[CC-BY-4.0](https://creativecommons.org/licenses/by/4.0/)**
